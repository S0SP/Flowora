...
export function DealsSettingsPanel() {
  ...
  return (
    <div className="max-w-[800px] animate-in fade-in-50 duration-200 space-y-6">
      <div>
        <h1 className="text-[22px] font-bold text-foreground mb-1">Deals & Currency Settings</h1>
      ...
      <div className="bg-white border border-border rounded-xl p-6 shadow-sm space-y-5">
        <div className="border-b border-border pb-3 flex items-center gap-2">
          ...
          <h3 className="text-[15px] font-semibold text-foreground">Default Workspace Currency</h3>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-2 sm:max-w-xs">
            <label className="text-[13px] font-semibold text-foreground">Currency Code</label>
            <select
              value={selectedCurrency}
              onChange={(e) => setSelectedCurrency(e.target.value)}
              disabled={!canEditSettings}
              className="h-10 w-full rounded-lg border border-border bg-white px-3 text-sm text-foreground outline-none focus:border-primary focus:ring-1 focus:ring-primary disabled:cursor-not-allowed disabled:opacity-60"
            >
              {CURRENCIES.map((c) => (
          ...
          {canEditSettings && (
            <div className="pt-2">
              <button
                ...
                className="px-6 py-2 bg-primary hover:bg-primary/95 text-foreground font-semibold text-[13px] rounded-lg shadow-sm disabled:opacity-50 transition-all flex items-center gap-1.5"
              >
                {saving ? (
                  <>
