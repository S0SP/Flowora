...
const dotColors: Record<string, string> = {
  new: "bg-teal-600 dark:bg-[#06B6D4]",
  contacted: "bg-blue-600 dark:bg-[#3B82F6]",
  qualified: "bg-amber-600 dark:bg-[#F59E0B]",
  proposal: "bg-indigo-600 dark:bg-[#8B5CF6]",
  won: "bg-emerald-600 dark:bg-[#22C55E]",
  lost: "bg-red-600 dark:bg-[#EF4444]",
}
...
function LeadCardComponent({ item, onEdit }: { item: LeadCard, onEdit: () => void }) {
  return (
    <div className="bg-white dark:bg-[#1D1F25] border border-border/60 dark:border-[#2A2E36] rounded-lg p-2.5 shadow-sm hover:shadow dark:shadow-none transition-all duration-200 cursor-grab active:cursor-grabbing group relative select-none">
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-2 min-w-0 pr-4">
          ...
          <div className="min-w-0">
            <h4 className="font-semibold text-xs text-gray-900 dark:text-[#F8FAFC] truncate leading-snug">{item.name}</h4>
            {item.source && (
              <p className="text-[9px] text-gray-500 dark:text-[#9CA3AF] flex items-center gap-1 mt-0.5 truncate">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button onPointerDown={(e) => { e.stopPropagation(); onEdit() }} className="text-gray-500 dark:text-[#9CA3AF] hover:text-gray-900 dark:hover:text-[#F8FAFC] opacity-0 group-hover:opacity-100 transition-opacity">
            {item.type === "converted" ? <Zap className="h-3.5 w-3.5 text-amber-500 fill-amber-500" /> : <MoreHorizontal className="h-3.5 w-3.5" />}
          </button>
        </div>
      </div>

      {item.phone && <p className="text-[10px] text-gray-500 dark:text-[#9CA3AF] mb-1.5 flex items-center gap-1"><Phone className="h-2.5 w-2.5" /> {item.phone}</p>}

      {item.tags && item.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-2">
          {item.tags.map((tag, idx) => (
            <Badge key={idx} className="bg-white dark:bg-[#17181D] border border-gray-200 dark:border-[#2A2E36] text-gray-500 dark:text-[#9CA3AF] hover:bg-gray-50 dark:hover:bg-[#17181D]/80 font-semibold px-1.5 py-0 min-h-[16px] text-[11px] rounded-[4px]">
              {tag}
            </Badge>
          ))}
        </div>
      ...
      {item.custom_fields && Object.keys(item.custom_fields).length > 0 && (
        <div className="space-y-0.5 mb-2">
          {Object.entries(item.custom_fields).map(([key, val]) => (
            <div key={key} className="bg-gray-100/30 dark:bg-[#17181D] rounded px-1.5 py-0.5 text-[9px] flex justify-between">
              <span className="text-gray-500 dark:text-[#9CA3AF] font-semibold uppercase">{key}</span>
              <span className="font-semibold text-gray-900 dark:text-[#F8FAFC]">{String(val)}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {item.latest_note && (
        <div className="bg-[#F9FAFB] dark:bg-[#17181D] border border-[#E5E7EB] dark:border-[#2A2E36] rounded-md p-1.5 mb-2 text-[10px] text-gray-500 dark:text-[#9CA3AF]">
          ...
          {item.followup_date && (
            <span className={`inline-flex items-center gap-1 text-[8px] font-bold px-1.5 py-0.5 rounded mt-1 ${item.followup_completed ? "bg-emerald-50 dark:bg-[#22C55E]/10 text-emerald-700 dark:text-[#22C55E]" : "bg-gray-100 dark:bg-[#1D1F25] text-gray-500 dark:text-[#9CA3AF]"}`}>
      ...
      {item.date && (
        <div className="bg-gray-50 dark:bg-[#17181D] rounded p-1 mb-2 flex flex-col gap-0.5 text-[10px] text-gray-900 dark:text-[#F8FAFC]">
          <span className="font-medium flex items-center gap-1"><Calendar className="h-2.5 w-2.5" /> {item.date}</span>
          {item.voice && (
            <div className="flex items-center gap-1 text-[9px] text-primary dark:text-[#22C55E] font-medium">
      ...
      <div className="flex items-center justify-between text-gray-500 dark:text-[#9CA3AF] border-t border-border/40 dark:border-[#2A2E36] pt-1.5 mt-1.5">
        {item.time && <span className="text-[9px]">{item.time}</span>}
        {item.value && <span className="text-[11px] font-bold text-gray-900 dark:text-[#F8FAFC]">{item.value}</span>}
        
        {item.type === "standard" && (
          <div className="flex gap-1.5 text-gray-500/50 dark:text-[#9CA3AF]/60">
        ...
        {item.type === "contacted" && !item.time && (
          <div className="flex items-center gap-2 text-[9px] font-medium text-gray-500/60 dark:text-[#9CA3AF]/60 w-full justify-between">
...
function SortableColumn({ col, items, onOpenAdd, onEditLead }: { col: ColumnData; items: LeadCard[], onOpenAdd: (status: string) => void, onEditLead: (id: string) => void }) {
  ...
  return (
    <div ref={(node) => { setSortableRef(node); setDroppableRef(node); }} style={style} className="flex flex-col h-full bg-gray-100/20 dark:bg-[#17181D] border border-border/40 dark:border-[#2A2E36] rounded-xl p-3 min-h-[500px] shadow-[inset_0_1px_2px_rgba(0,0,0,0.01)] dark:shadow-none transition-colors">
      <div className="flex items-center justify-between mb-3 pb-1 cursor-grab active:cursor-grabbing shrink-0" {...attributes} {...listeners}>
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-[13px] text-gray-900/90 dark:text-[#F8FAFC]">{col.title}</h3>
        </div>
        <Badge className="bg-gray-100 dark:bg-[#1D1F25] text-gray-500 dark:text-[#9CA3AF] border-none font-semibold px-1.5 py-0 h-4 text-[10px] rounded hover:bg-gray-100/80 dark:hover:bg-[#2A2E36]">{items.length}</Badge>
      </div>
      <div className="flex-1 overflow-y-auto space-y-2 pb-4 min-h-[150px] flex flex-col">
        ...
        {items.length === 0 && (
          <div className="flex-grow border border-dashed border-border/65 dark:border-[#2A2E36] rounded-lg p-6 flex flex-col items-center justify-center text-center bg-gray-100/5 dark:bg-transparent min-h-[120px] transition-colors select-none">
            <span className="text-[10px] font-medium text-gray-500/60 dark:text-[#9CA3AF]/60">No leads in stage</span>
          </div>
        )}
        <button onClick={() => onOpenAdd(col.id)} className="w-full py-1.5 flex items-center justify-center gap-1.5 text-[11px] font-semibold text-gray-500 dark:text-[#9CA3AF] hover:text-gray-900 dark:hover:text-[#F8FAFC] hover:bg-gray-50 dark:hover:bg-[#1D1F25] rounded-lg border border-transparent transition-colors mt-2 shrink-0">
